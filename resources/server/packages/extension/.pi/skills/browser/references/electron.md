<!--
Vendored from agent-browser upstream via `agent-browser skills get electron`
at CLI version 0.27.0 (upstream tag v0.27.0,
commit c830d1b67dc18b754e305859f0ae587f858a1447).
Frontmatter stripped — this is a reference doc, not a standalone skill.
License: Apache-2.0 (see ../LICENSE).
See ../UPSTREAM.md for refresh procedure.
-->
...
The Pi Dashboard Electron app supports an opt-in CDP debug surface so you can
drive the *installed shell* — not just a separate Chrome aimed at
`http://localhost:8000`. The wizard window, doctor window, tray menu, native
menus, and IPC are all reachable this way.

> **Single-instance contract**: enabling CDP requires fully quitting and
> relaunching the app. Passing `--debug-cdp` to a running instance only
> logs a warning to the running instance's stderr — Chromium stands up the
> CDP server during browser-process init, with no API to enable it later.

### 1. Launch with CDP enabled

The app accepts either an `--debug-cdp` CLI flag or a `PI_DEBUG_CDP` env var.
The flag takes precedence; default port is `9222`.

**macOS:**

```bash
# Quit any running instance first.
osascript -e 'quit app "PI Dashboard"'

# Default port 9222
open -a "PI Dashboard" --args --debug-cdp

# Custom port
open -a "PI Dashboard" --args --debug-cdp=9333
```

**Linux / Windows (env-var equivalent):**

```bash
PI_DEBUG_CDP=1 /path/to/pi-dashboard            # default port 9222
PI_DEBUG_CDP=9444 /path/to/pi-dashboard          # custom port
```

**Dev (in this repo):**

```bash
cd packages/electron && npm run dev:cdp
```

On activation the app logs a single warning line to its stderr:

```
[debug-cdp] CDP listening on :9222 — local automation is enabled
```

If this line is missing, CDP is not active — the agent should halt the
recipe and tell the user to relaunch with the flag.
...
### 3. List the Electron windows

```bash
agent-browser tab
```

You should see the main dashboard window plus, depending on what the app
has open, the wizard window and/or doctor window:

```
0  PI Dashboard                      http://localhost:8000/
1  PI Dashboard — Welcome            file:///.../wizard.html
2  PI Dashboard — Diagnostics        file:///.../doctor.html
```

Switch tabs with `agent-browser tab <index>`.

### 4. Capture a screenshot of the main window

```bash
agent-browser tab 0
agent-browser screenshot pi-dashboard.png
```

### 5. Drive the wizard window

```bash
agent-browser tab 1
agent-browser snapshot -i      # find the "Launch dashboard" button
agent-browser click @e3
agent-browser screenshot wizard-after-launch.png
```

### 6. Drive the doctor window

```bash
agent-browser tab 2
agent-browser snapshot -i
agent-browser screenshot doctor.png
```
...
This disconnects the agent-browser session but does **not** quit the Pi
Dashboard app — it remains running with CDP still active for the next
connection. To end CDP entirely, quit the Pi Dashboard app.
